...
export async function generateMetadata(props: {
  ...
  try {
    const tag = await getTagBySlug(params.slug, page);
...
export default async function TagPage(props: { 
  ...
  try {
    const tag = await getTagBySlug(params.slug, page, limit);

    if (!tag) {
      // Check for redirect
      const currentPath = `/tag/${params.slug}`;
      ...
      return (
        ...
        <h1 className="text-3xl font-bold text-gray-800">Tag Not Found</h1>
    ...
    <div className="max-w-6xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-12 gap-8 bg-white">
      ...
      <div className="lg:col-span-8">
        <h1 className="text-[28px] sm:text-[36px] lg:text-[40px] xl:text-[48px] font-bold mb-6 text-gray-800 border-b pb-2 uppercase">
          Tag: <span className="text-[#568c2c]">#{tag.name}</span>
        </h1>

        {tag.posts.length === 0 ? (
          <p className="text-gray-500 italic">No posts found with this tag yet.</p>
        ) : (
